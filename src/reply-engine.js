const DEFAULT_AI_SYSTEM_PROMPT = "根据原推文写一句自然的中文回复。像真实用户刷到后随手留下的感受，简短、有一点具体反应，不必完整表达观点。5到20个汉字为主，可保留必要的英文词。避免宣传腔、总结腔、夸张吹捧、复述原文和模板化感叹。只输出回复。";
...
async function generateLighthouseAIReply(aiConfig, tweet, options = {}) {
  ...
  const basePrompt = appendEventReplyGuardInstruction(
...
function normalizeProvider(provider) {
  return AI_PROVIDER_CONFIG[provider] ? provider : "gpt-5.6-terra";
}
...
async function pickUserFallbackReply(seed, systemPrompt = DEFAULT_AI_SYSTEM_PROMPT) {
  if (!USER_FALLBACK_REPLIES.length) {
  ...
  if (storageArea) {
    try {
      const stored = await storageArea.get(["lighthouseFallbackReplyBag"]);
      const storedBag = Array.isArray(stored?.lighthouseFallbackReplyBag)
        ? stored.lighthouseFallbackReplyBag.filter((reply) => USER_FALLBACK_REPLIES.includes(reply))
        : [];
      if (storedBag.length) fallbackReplyBag = storedBag;
    } catch (_) {}
  }

  if (!fallbackReplyBag.length) refillFallbackReplyBag();

  let reply = takeNextValidFallbackReply(systemPrompt);
  if (!reply) {
    refillFallbackReplyBag();
    reply = takeNextValidFallbackReply(systemPrompt);
  }
  if (!reply) {
  ...
  if (storageArea) {
    try {
      await storageArea.set({ lighthouseFallbackReplyBag: fallbackReplyBag });
    } catch (_) {}
  }
  return reply;
}

function refillFallbackReplyBag() {
  fallbackReplyBag = [...USER_FALLBACK_REPLIES];
...
function takeNextValidFallbackReply(systemPrompt) {
  while (fallbackReplyBag.length) {
    const candidate = fallbackReplyBag.pop();
    if (validateFinalReplyText(candidate, systemPrompt).ok) return candidate;
  }
  return "";
}
